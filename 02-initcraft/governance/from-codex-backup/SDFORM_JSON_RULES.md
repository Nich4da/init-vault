...
## 12. ภาคผนวก 2026-09-10 · component ที่ "ไม่มีแม่แบบ" คือกับดัก (เคส `checkbox-input`)

เพิ่มจากเคสจริง: ฟอร์ม `xray-contrast-media-record-v1.json` — **checkbox หายทั้ง 5 กลุ่ม**
ทั้งกลุ่มที่ไม่ได้ซ่อนและกลุ่มที่ต้องโผล่ตอนกดปุ่ม Yes · widget ชนิดอื่นในฟอร์มเดียวกันขึ้นครบ

### 12.1 อาการนี้ไม่ใช่บั๊กของสคริปต์ซ่อน/แสดง

ถ้ากลุ่มที่ `hidden:false` ตั้งแต่แรกก็ไม่ขึ้นด้วย แปลว่า widget ไม่ถูกเรนเดอร์ตั้งแต่ต้น
**ห้ามไปไล่แก้ `onChange` / `setHidden`** ให้กลับมาดูชุด `options` ก่อนเสมอ

### 12.2 แม่แบบต้องเป็น "ของระบบ" เท่านั้น — ไฟล์ที่เราเขียนเองไม่ใช่แม่แบบ

`checkbox-input` **ไม่มีในฟอร์มแม่แบบของระบบเลยสักไฟล์** ชุด 22 คีย์ที่เคยใช้ลอกมาจาก
`SDForm/sdform_module/lab_order_mock_import.json` ซึ่ง *เราเขียนขึ้นเอง* และไม่เคยพิสูจน์ว่า
import แล้วขึ้นจริง ⇒ กลายเป็น "แบบอย่างปลอม" ที่ทำให้ทั้ง validator และเทสยืนยันของผิดให้

> **กติกา:** ไฟล์ที่จะนับเป็นแม่แบบได้ ต้องเป็นฟอร์มที่ระบบ export มา (`sdform_module/`)
> หรือฟอร์มที่ผู้ใช้ยืนยันแล้วว่าเปิดในระบบจริงได้ · งานร่างของเราเองห้ามใช้อ้างอิงเด็ดขาด

### 12.3 ไม่มีแม่แบบของตัวเอง ⇒ ใช้ "วิดเจ็ตพี่น้อง"

`radio-input` มีแม่แบบของระบบ 13 ตัว (`patient.json` · `visit.json` · `disease.json` ฯลฯ)
ทุกตัวมีชุดคีย์เดียวกัน **29 คีย์** ตรงกับ `SNAPSHOT['radio-input']` ของ validator
ชุด 22 คีย์ของ checkbox จึง **ขาดไป 7 คีย์**:

```
labelAlign · validation · validationHint · labelIconClass · labelIconPosition
labelTooltip · labelColor
```

⇒ ตรงกับเงื่อนไข **ก** (options ไม่ครบ ⇒ ไม่ render) พอเติมให้ครบ 29 คีย์เท่าแม่แบบ radio
โครงก็ถูกต้องตามกฎทุกข้อ · `check_sdform_json.py` เพิ่ม `SIBLING = {'checkbox-input': 'radio-input'}`
ให้เทียบข้ามวิดเจ็ตพี่น้องได้แล้ว จากเดิมที่ได้แค่เตือนว่า "ไม่มีแม่แบบให้เทียบ" แล้วปล่อยผ่าน

### 12.4 `customClass` ของระบบเป็น **อาร์เรย์** เสมอ

สแกนทั้งรีโป: ค่าที่ไม่ว่างซึ่งระบบสร้างเองเป็นอาร์เรย์ทุกตัว (`["mb15"]` · `["lab-waiting-listview"]`)
ส่วนค่า **string** ที่ไม่ว่างพบเฉพาะในไฟล์ที่เราพิมพ์เอง และไฟล์กลุ่มนั้นคือไฟล์ที่ไม่เรนเดอร์
(`card` ของฟอร์มนี้ · `file-upload-input` ของ `Result_Report_Manual_UI_*`)

⇒ ต้องการคลาสไว้ให้ CSS เกาะ ให้ใช้ที่ระดับฟอร์ม `formConfig.customClass: ['ชื่อคลาส']`
(แบบอย่างของระบบ: `person_form.json` ใช้ `["mb15"]`) อย่าไปใส่ค่าลง `options.customClass` ของ widget

### 12.5 ข้อสรุปเดิมที่ถูกยกเลิก

บันทึกเมื่อ 2026-09-10 รอบสามเคยสรุปว่า "**คีย์เกิน** ทำให้ checkbox หายทั้งกลุ่ม" แล้วถอด
`validation` / `labelTooltip` / `labelColor` ออกจาก checkbox — **ผิด และเป็นสิ่งที่ทำให้กลุ่มหาย**
ของจริงคือ *คีย์ขาด* ตามเงื่อนไข ก · เก็บบันทึกนี้ไว้เพื่อไม่ให้ใครสรุปวนกลับไปทางเดิมอีก
