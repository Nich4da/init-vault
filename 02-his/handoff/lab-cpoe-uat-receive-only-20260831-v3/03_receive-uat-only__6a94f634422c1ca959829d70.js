 ...
 * - UAT default is receive-only: persist receipt and LAB NO. without Agent.
 *   Pass submit_agent:true only when transport testing is intentionally enabled.
...
const submitAgent = Boolean(params && params.submit_agent === true)
...
if (!submitAgent) {
  await itemCollection.updateOne(
    { _id: itemObjectId, xrstatx: active },
    {
      $set: {
        hl7_status: 'new',
        agent_transport_state: 'uat_receive_only',
        agent_retryable: false,
        agent_error: '',
        agent_error_reason: 'UAT รับ specimen ใน HIS เท่านั้น; ยังไม่ส่ง Agent',
        updated_at: now,
        updated_by: actorCode
      }
    }
  ...
  return {
    ...
    message: 'รับ specimen แล้ว (UAT: ยังไม่ส่ง Agent)'
  }
}
if (siblingQueued) {
  await itemCollection.updateOne(
