...
| Change existing work  | (any edit to working code)    | §14 — check what already works first |
...
## 14. Regression safety — ห้ามทำของเดิมที่ใช้งานได้พัง

**กฎที่ผู้ใช้ตั้งไว้ 2026-09-02** หลังต้องวนกลับมาแก้ของเก่าซ้ำหลายรอบ

> "เช็คก่อนว่าของเดิมอะไรที่ถูกอยู่แล้ว ทำงานได้อยู่แล้ว ห้ามแก้
> หากจำเป็นต้องแก้ ให้แก้โดยไม่กระทบอันเก่าที่ทำงานได้
> เวลาส่งงานใหม่มา ฟังก์ชันเก่าต้องห้ามหายไป"

กฎนี้บังคับกับงานทุกชนิดในรีโปนี้ ไม่ใช่แค่โค้ด — ฟอร์ม, API, สคริปต์, เอกสาร, สคีมา

### 14a. ก่อนแก้ — ระบุของเดิมที่ใช้งานได้ก่อนเสมอ

1. อ่านของเดิมให้เข้าใจก่อนแตะ **ห้ามแก้จากการเดา**
2. ไล่ให้ครบว่าส่วนไหน "ทำงานได้แล้วจริง" — ยืนยันด้วย runtime evidence, เทสที่ผ่านอยู่,
   หรือผู้ใช้เคยยืนยันแล้วในบทสนทนา
3. งานใหม่ต้อง **เพิ่มเข้าไป** (additive) เป็นค่าเริ่มต้น: ฟิลด์ใหม่ · สาขาเงื่อนไขใหม่ ·
   ฟังก์ชันใหม่ — เส้นทางเดิมต้องเดินได้เหมือนเดิมโดยไม่ต้องแก้อะไร

### 14b. เทสที่ผ่านอยู่คือสัญญา ไม่ใช่อุปสรรค

**ห้ามลบหรือลดความเข้มของ assertion เดิมเพียงเพื่อให้ของใหม่ผ่าน** — นี่คือสาเหตุ
ที่ฟังก์ชันเก่าหายไปเงียบ ๆ บ่อยที่สุด

เทสเก่าแดงขึ้นมา แปลว่าอย่างใดอย่างหนึ่งเท่านั้น:

| เทสเก่าแดงเพราะ | ต้องทำ |
|---|---|
| ของใหม่ไปทำของเก่าพัง | **แก้ของใหม่** ไม่ใช่แก้เทส |
| พฤติกรรมเดิมต้องเปลี่ยนจริงตามที่ผู้ใช้สั่ง | **บอกผู้ใช้ก่อนว่ากำลังจะเปลี่ยนอะไร** แล้วค่อยแก้เทส พร้อมคอมเมนต์ว่าใครสั่งและเมื่อไหร่ |
| เทสเดิมผูกกับรายละเอียดที่ไม่ใช่สัญญาจริง | แก้ได้ แต่ต้องคงสิ่งที่มันปกป้องไว้ในรูปแบบอื่น |

ทุกครั้งที่แก้ assertion เดิม ต้องเขียนเหตุผลไว้ในเทสว่าเปลี่ยนเพราะอะไร ลงวันที่กำกับ

### 14c. หลังแก้ — ต้องพิสูจน์ว่าของเดิมยังอยู่

1. รัน **ชุดเทสทั้งหมดที่เกี่ยวข้อง** ไม่ใช่แค่ตัวที่เพิ่งเขียน
2. รัน validator ที่เกี่ยวข้องทุกตัว
3. ในข้อความส่งงาน ต้องบอกให้ชัดว่า **อะไรเปลี่ยน** และ **อะไรที่ยังเหมือนเดิม**
   ห้ามให้ผู้ใช้ต้องไปไล่หาเอง

### 14d. สิ่งที่ต้องขออนุญาตก่อนเสมอ

- เปลี่ยนพฤติกรรมเดิมที่ผู้ใช้เคยยืนยันแล้วว่าใช้งานได้
- ลบฟิลด์ · ปุ่ม · action · ฟังก์ชัน ที่มีอยู่แล้ว
- เปลี่ยนความหมายของค่าที่ระบบอื่นอ่านอยู่ (contract ของ API/ฟอร์ม)

ถ้าไม่แน่ใจว่านับเป็นการเปลี่ยนของเดิมหรือไม่ — **ถือว่าใช่ แล้วถามก่อน**

### 14e. เอกสารและ Hot Cache ก็อยู่ใต้กฎนี้

การเขียนทับ `spec.md` / `design.md` / `hotcache.md` ต้องรักษาข้อมูลเดิมที่ยังจริงอยู่
บีบให้สั้นได้ แต่ห้ามทำให้ข้อเท็จจริงที่ยังใช้ได้หายไป และห้ามลบบันทึกของ session อื่น
