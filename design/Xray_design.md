...
อัปเดต: 2026-09-24 (เชื่อมสถานะการเงินและเพิ่มแท็บสืบค้นผล LAB แบบอ่านอย่างเดียว)
...
| 4 | การรับรายการ | รับ specimen ราย test ด้วย checkbox → ตรวจ specimen master → สร้าง Lab No. → ส่ง Agent | ไม่มีด่าน specimen; มี checkbox ราย item, เลือกหลายรายการหรือเลือกทั้งหมด แล้ว UI ส่งเข้าเครื่องตามลำดับทีละ item |
...
| 9 | หน่วยของการทำงาน | Item — รับ/ปฏิเสธ ราย test ได้ | **Item ภายใต้ Order** — หนึ่ง Order มีหลาย X-ray item ได้; ส่ง/ยกเลิกตามรายการที่เลือก |
...
| 10 | ปุ่มใน detail | `รับ specimen` · `ปฏิเสธรายการที่เลือก` · `ยกเลิก order` | **`ส่งเข้าเครื่อง` · `ปฏิเสธรายการ` · `ยกเลิกรายการที่เลือก`** |
| 11 | PDF ของใบที่ยกเลิก | แสดงปุ่ม PDF | **ไม่แสดงปุ่ม PDF** |

ทุกข้ออื่นให้ถือว่าเหมือน LAB

### 0.1 แท็บ `สืบค้นผลแลป` สำหรับผู้ใช้ X-ray (เพิ่ม 2026-09-24)

- วางเป็นแท็บระดับบนลำดับที่ 4 ระหว่าง `X-ray` กับ `Completed`; ไม่แทนที่และไม่เปลี่ยน
  flow ของ `Unit Queue`, `My Room`, worklist X-ray หรือ RIS
- เปิดแท็บแล้วต้องยังไม่ query จนกว่าจะระบุ **HN แบบตรงตัว** หรือเลือก Date Range ครบทั้ง
  วันเริ่มและวันสิ้นสุด เพื่อไม่ให้เกิดคำขอดึงผู้ป่วยทุกคนโดยไม่ตั้งใจ
- ถ้าระบุ HN แต่ไม่ระบุวันที่ ให้ส่ง `all_dates:true`; ถ้าเลือก Date Range ให้ส่ง
  `date_from`/`date_to` ตามช่วงนั้น
- แสดงได้ทั้ง `ผลห้องแลป` (`lookup_mode:'results'`) และ `รายการที่สั่ง`
  (`lookup_mode:'orders'`) จาก **ทุก LAB section ที่เปิดใช้งาน** ด้วย `cross_section:true`
- เรียก LAB Worklist Process `6a9434c3422c1ca959829d5e` ผ่าน `runProcess` เท่านั้น:
  ใช้ได้เฉพาะ `action:'list'` และ `action:'get_manual_result'`
- หน้า X-ray เป็น **read-only** ต่อข้อมูล LAB: ห้ามมีรับ specimen, ปฏิเสธ, บันทึก/แก้ผล,
  แนบไฟล์ หรือเปลี่ยน visibility ของผล และ server ต้องปฏิเสธ write action เมื่อ
  `cross_section:true`
- การ deploy ต้องให้กลุ่มผู้ใช้ X-ray มีสิทธิ์เรียก LAB Process นี้ และ UAT ด้วยบัญชี X-ray
  ว่าเห็นทุกห้องตามขอบเขตที่ server อนุญาต โดยไม่เห็น action เขียนใด ๆ
...
X-ray ใช้ semantic color เดิมและเพิ่มสีส้มกลางสำหรับช่วงรอผลตรวจ:

| จุดใช้งาน | Border | Background | Text |
|---|---|---|---|
| รอรับ (ยังไม่ส่งเข้าเครื่อง) | `#fadb14` | `#fffbe6` | `#8a6d00` |
| รอผลตรวจ (ส่งเข้าเครื่องแล้ว) | `#e6a23c` | `warning-50` | `#b88230` |
| ออกผลแล้ว / ออกผลครบ | `success-200` | `success-50` | `#529b2e` |
...
- Dark mode: ป้ายความเร่งด่วน, หน่วยต้นทาง, การชำระเงิน, เลข Order และเครื่องต้องใช้
  Element Plus theme variables; ป้าย `รอรับ` คงเหลือง `#FADB14` แต่ใช้พื้นโปร่งเข้มและข้อความ
  เหลืองอ่อน เพื่อไม่เกิดกล่องพื้นขาวจ้า
...
92px                  /* LAB = จำนวน specimen → X-ray = ตัวย่อเครื่อง */
126px
100px
minmax(110px,.95fr)
84px
...
- คอลัมน์ `เครื่อง` แสดงตัวย่อของทุกเครื่องใน Order เป็น tag แยกตามลำดับ item
...
8. **ปฏิเสธ** (เหตุผล)
9. **คนปฏิเสธ**
10. **สติ๊กเกอร์ HN** — PDF ระดับ item ที่มี HN และ Accession No.

- **Accession No. อยู่ที่ตารางระดับ test เท่านั้น ไม่ใช่ระดับ Order** (ผู้ใช้ยืนยัน 2026-08-31)
  ก่อนส่งเข้าเครื่องให้แสดงค่าว่าง ไม่ใช้ `–`; เมื่อมีค่าแสดงด้วย mono/tabular numerals
...
- มี checkbox ราย item และ checkbox ที่หัวคอลัมน์สำหรับเลือก/ล้างรายการที่ดำเนินการได้ทั้งหมด
  `สติ๊กเกอร์ HN` เป็น document action ระดับ item เท่านั้น
...
- เลือกได้หลายค่า; แสดงตัวย่อ code ครบทุกค่าที่เลือกและห้ามยุบเป็น `+N`
- แถวละไม่เกิน **3 เครื่อง**; ตัวที่ 4 เป็นต้นไปขึ้นบรรทัดใหม่ ช่องกรองใช้ความกว้างฐานเดียวกันสำหรับ 0–3 code สั้น ไม่กว้างเกินเมื่อเลือกแค่ 2 และขยายเฉพาะเมื่อ code ยาวจริง ไม่โตตามจำนวนแถว (หน้าจอแคบมากลดเหลือ 2 คอลัมน์)
- ก่อนเลือก ค่า `Select all` ต้องอยู่กึ่งกลางแนวตั้งตาม `el-select` ปกติ; เมื่อมี tag จึงชิดบนเพื่อให้กล่องสูงลงล่าง
- พื้นหลังตัว control และรายการใน dropdown คงสีขาว/เทาตาม Element Plus เดิม; ใช้สีฟ้าเฉพาะ tag ที่เลือกใน textbox
...
แถบ status chip ต้องมี legend แบบมองเห็นได้โดยไม่ต้อง hover อยู่ด้านขวา: สีเหลืองอ่อน = `รอรับ`, สีส้ม = `รอผลตรวจ`, สีเขียว = `ออกผลแล้ว`, สีแดง = `ยกเลิก` โดยใช้สีชุดเดียวกับจุดในคอลัมน์สถานะ
...
- ใน detail panel มีปุ่ม **`ส่งเข้าเครื่อง`**, **`ปฏิเสธรายการ`**,
  **`ยกเลิกรายการที่เลือก`** และ **`ตรวจใหม่เฉพาะที่เลือก`** ตามสถานะรายการในใบ
- `ส่งเข้าเครื่อง` ปุ่มเขียว ทำงานกับรายการที่ติ๊กไว้; เลือกหลายรายการได้และ UI เรียก Process
  เดิมตามลำดับทีละ item เพื่อคง contract ฝั่ง server
- `ปฏิเสธรายการ` ใช้กับรายการที่ยังไม่ส่งเข้าเครื่องตาม Form/Process ที่มีอยู่
- **แก้ 2026-09-18 ตามคำยืนยันผู้ใช้:** `ยกเลิกรายการที่เลือก` ส่ง `item_ids` จาก
  checkbox เดียวกัน ยกเลิกเฉพาะรายการเหล่านั้น; sibling ที่มีผลหรือไม่ได้เลือกไม่ถูกแตะ
  และไม่มี selection แล้วปุ่ม disabled. รายการที่มี Accession ต้องให้ API Order ทีม
  ยืนยัน `LocalSaved:true` ของ Accession เดิมพร้อม `IsDeleted:true` ก่อนเปลี่ยนสถานะใน HIS;
  ถ้าทีมส่งต่อ RIS ไม่สำเร็จให้เตือนและเก็บ HTTP status แยกใน audit (ผู้ใช้ยืนยัน 2026-09-18)
- `ตรวจใหม่` ใช้ **Order เดิมและเลข Order เดิม** ไม่สร้างใบใหม่ (2026-09-03);
  **แก้ 2026-09-18:** เลือก X-ray item ที่ยกเลิกแล้วในแท็บ order ก่อนเสมอ แม้ใบมีเพียง
  บางรายการที่ยกเลิก. API รับ `item_ids` บังคับและเปิดกลับเฉพาะที่เลือก;
  sibling คงเดิม. Form เรียก `retest_items` เพื่อไม่ให้ Process เก่าตีความเป็นตรวจใหม่ทั้งใบ.
  ล้าง Accession เฉพาะรายการที่เปิด แล้วออกเลขใหม่ตอนกด `ส่งเข้าเครื่อง`
...
- แถว Order ที่ไม่ยกเลิกมี `PDF` ใบสั่งตรวจและ `HN` ป้ายติดแฟ้มผ่าน Report Factory ก่อน `EMR`; ส่ง scope
  `order_id` + `visit_id` ของ Order ที่กด และ disable พร้อมเหตุผลเมื่อ scope ไม่ครบ
- ปุ่ม `HN` ใช้ Report `ป้ายติดแฟ้ม (HN)` `6a9a355c422c1ca95982a1a2` ระดับ Order และส่ง `hn` ของแถวที่กด (เปลี่ยนจาก `5256d813009293b480d0a15c` + `xparentx` ตามคำสั่งผู้ใช้ 2026-09-04; หน้า LAB ยังใช้ตัวเดิม)
- `PDF` ใบสั่งตรวจระดับ Order เป็นคนละเอกสารกับ `PDF ผลอ่าน` ระดับ item; ผลอ่านยังปลดล็อก
  เมื่อ item นั้นมีผลแล้วเท่านั้น
- คอลัมน์ท้ายของ item มี `สติ๊กเกอร์ HN` ผ่าน Report Factory; เปิดได้ก่อนมี Accession No.
  เมื่อมีเลขต้องแสดงของ item ที่กด และ scope ต้องครบ `order_id + visit_id + item_id`
  ขนาด 8.5 × 2 ซม. ใช้คำว่า `วันเกิด` แทน `DOB`
...
- Search placeholder: `ค้นหา HN / VN / ON / AN / ชื่อ`; `ON` = Order No. และ `AN` = Accession No. โดย `aria-label` ต้องคงคำเต็ม
...
- Filters: `ทั้งหมด`, `รอรับ`, `รอผลตรวจ`, `ออกผลครบ`, `ยกเลิก`
- Order actions: `ส่งเข้าเครื่อง`, `ปฏิเสธรายการ`, `ยกเลิกรายการที่เลือก`
- Context action: `EMR`
...
- Transport: สถานะหลักหลังส่งใช้ `รอผลตรวจ`; รายละเอียดการส่งใช้ `ส่งเครื่องแล้ว` / `ส่งเครื่องไม่สำเร็จ`
...
- checkbox ราย item ต้องเลือกหลายรายการและเลือกทั้งหมดได้; การส่ง batch ที่ UI ต้องยังเรียก
  Process ราย item ตามลำดับ ไม่เปลี่ยน server contract เป็น payload หลายรายการ
...
- ห้ามแสดงปุ่ม PDF ทั้งใบสั่งตรวจและผลอ่านบนใบที่ยกเลิกแล้ว
...
| รอรับ | ยังไม่ส่งเครื่อง (`sent` = ยังไม่ผ่านการเงิน, `ready` = ผ่านการเงินแล้ว แต่ยังไม่ได้รับเข้า) |
| รอผลตรวจ | ส่งเครื่องบางรายการ, ส่งเครื่องแล้วรอผล, ออกผลบางส่วน |
...
### Finance gate — เหมือน LAB (ยืนยัน 2026-09-24)

- `sent` และ `ready` ต้องยังแสดงและนับในกลุ่ม **รอรับ** เหมือนกัน เพื่อไม่ให้การเงิน
  เปลี่ยนภาพรวมของงานรังสีโดยไม่จำเป็น
- `ready` คือสถานะที่การเงินอนุมัติแล้ว จึงออก Accession No. และกด `ส่งเข้าเครื่อง`
  ครั้งแรกได้; `sent` ยังไม่ผ่านการเงิน ปุ่มคงมองเห็นแต่ disabled พร้อม tooltip
  **`ยังไม่ผ่านการเงิน`**
- รายการ `sent` ยังเลือกได้เพื่อ `ปฏิเสธรายการ` หรือ `ยกเลิกรายการที่เลือก`; gate นี้ปิดเฉพาะ
  การรับเข้า/ออก Accession/ส่ง RIS
- ฝั่ง server ต้อง fail closed ก่อนจองเลขหรือเปลี่ยนสถานะ: ทั้ง Accession Process และ Dispatch
  Process ต้องปฏิเสธ `sent`. รายการที่เคยรับแล้วและมีหลักฐาน dispatch, retry transport,
  ซ่อมรายการค้างครึ่งทาง หรือส่งตรวจซ้ำ ยังทำต่อได้ตามกติกาเดิมโดยไม่ย้อนกลับไปติด gate
...
1. CPOE สร้าง Order พร้อม `order_no` ตามรูปแบบใน Appendix F และผูกเครื่องปลายทางไว้ตั้งแต่ตอนสั่ง
   รายการเริ่ม `sent`; การเงินเปลี่ยนเป็น `ready` เมื่ออนุมัติแล้ว ทั้งสองสถานะแสดงเป็น `รอรับ`
...
6. การปฏิเสธราย item ทำได้เฉพาะก่อนส่งเข้าเครื่องตาม Process ปฏิเสธที่มีอยู่
7. การยกเลิกทำเฉพาะ X-ray item ที่ติ๊ก; บันทึกเหตุผล/ผู้ทำ/เวลาแยกตามชุดที่เลือก
   และถือว่า Order ถูกยกเลิกทั้งใบเมื่อทุก X-ray item ถูกยกเลิก/ปฏิเสธแล้ว
8. `ตรวจใหม่` / `ส่งตรวจซ้ำ` — **แก้ 2026-09-03 ตามคำสั่งผู้ใช้** (เดิม: สร้าง Order ใหม่ที่ลิงก์กลับ)
   ใช้ Order เดิมและเลข Order เดิม ไม่สร้างใบใหม่ · มีเลข Accession อยู่ให้ล้างทิ้งแล้วออกเลขใหม่
   ตอนกดส่งเข้าเครื่อง · เลขเดิมเก็บไว้ที่ `accession_history[]` เพราะผลอ่านรอบก่อนผูกกับเลขนั้น
   · **2026-09-18:** `ตรวจใหม่` ทำเฉพาะรายการที่เลือก (`item_ids`) ไม่เปิดกลับรายการอื่นใน Order
   · ข้อยกเว้น: รายการที่ `transport` ล้มเหลวค้างอยู่ยังใช้เลขเดิม เพราะ RIS ยังไม่เคยได้รับใบ
...
- ส่งเครื่องแล้วยังไม่มีผล → `รอผลตรวจ` สีส้ม (ถ้า transport ล้มเหลวให้เพิ่มป้าย `ส่งเครื่องไม่สำเร็จ`)
- มีผลอ่านแล้ว → `ออกผลครบ`
- ยกเลิก → `ยกเลิก` และไม่เข้าการนับข้างต้น

**`ออกผลบางส่วน` เกิดขึ้นไม่ได้ในโมเดลนี้** หาก contract เปลี่ยนเป็นหลายรายการต่อ Order
ให้ยังจัดอยู่ในกลุ่ม `รอผลตรวจ` จนกว่าทุกรายการจะออกผลครบ
...
ส่งเข้าเครื่อง (ราย Item)
  → xray-cpoe-dispatch API
    → ตรวจ Finance (`ready`) → ออก Accession → บันทึกเวลาส่ง/ผู้ส่ง
      → เรียก Agent submit → เก็บ transport outcome
...
| X9 | การยกเลิกหลังส่งเครื่องแล้ว | ปรับตามคำยืนยันผู้ใช้ 2026-09-18: ยกเลิกเฉพาะ item ที่เลือก; sibling ไม่ถูกแตะและผลของ sibling ไม่บล็อก. ยังไม่มี Accession ยกเลิกใน HIS ตามเดิม; มี Accession แต่ยังไม่มีผล ส่ง Order JSON ไป `xray_api_order`/GetOrder เส้นเดิมด้วยเลขเดิมและ `IsDeleted:true` รอ ACK ก่อนประทับ HIS; item ที่เลือกและมีผลแล้วห้ามยกเลิก · ต้อง UAT ว่า RIS เอาใบออกจากคิวจริง |
...
- [ ] checkbox ราย item เลือกหลายรายการและเลือกทั้งหมดได้; ไม่เลือก item แล้วห้ามส่ง
- [ ] ไม่มีบรรทัดสรุปคั่นระหว่างหัว Order กับตาราง
- [ ] ตัวกรองและคอลัมน์เครื่องแสดงตัวย่อครบทุก code ไม่ใช้ `+N`; ช่องไม่มีข้อมูลเป็นค่าว่าง
