...
sources: []
source_file: "03-source-materials/his-system-flow.drawio"   # not yet in 03-source-materials/ — see note below
source_type: image
source_date: unknown
author: unknown
...
> the user provided (the `.drawio` lives on Google Drive; not yet copied into `03-source-materials/`).
...
- **Provenance gap:** the source `.drawio` (Google Drive `1cN65pFSZG1zCya3A5Wm53LIfUHXjnF89`)
  is **not yet in `03-source-materials/`** — the connector token was expired at ingest time. This page is read
  from a screenshot; copy the real file to `03-source-materials/` when Drive access is restored.
